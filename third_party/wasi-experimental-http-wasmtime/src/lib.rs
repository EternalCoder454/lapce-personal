...
impl HttpState {
    ...
    // Local modification (Lapce Personal): wasmtime 14.0.4 tightened `Linker::func_wrap`
    // to require the store data type to outlive `'static`. Adding `T: 'static` here
    // satisfies that bound for all the `func_wrap` calls below.
